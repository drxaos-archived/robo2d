...
public class FaceParser extends LineParser {
    ...
    public void parse() {
        ...
        if (words.length == 4) {
            ...
            parsePoly(words.length - 1);
        } else {
            throw new RuntimeException("Could not identify face around '" + Arrays.toString(words));
        }
    }
    ...
    private void parsePoly(int vertices) {
        face.setType(Face.GL_TRIANGLE_FAN);
        parseLine(vertices);
    }
