...
public class PlatformModel {
    ...
    public Spatial createPlatform(PlatformImpl platform) {
        ...
        for (int i = 0; i < triangulation.size(); i++) {
            ...
            vertices[0] = new Vector3f((float) points.get(0).getY(), H / 5, (float) points.get(0).getX());
            vertices[1] = new Vector3f((float) points.get(1).getY(), H / 5, (float) points.get(1).getX());
            vertices[2] = new Vector3f((float) points.get(2).getY(), H / 5, (float) points.get(2).getX());
        ...
        for (int i = 0; i < vert.size(); i++) {
            ...
            double a01 = (a0 + a1) / 2;
            double a12 = (a1 + a2) / 2;
            vertices[0] = new Vector3f((float) v1.getY() + FastMath.sin((float) a01) * 2, -H * 4 / 5, (float) v1.getX() + FastMath.cos((float) a01) * 2);
            vertices[1] = new Vector3f((float) v2.getY() + FastMath.sin((float) a12) * 2, -H * 4 / 5, (float) v2.getX() + FastMath.cos((float) a12) * 2);
            vertices[2] = new Vector3f((float) v2.getY(), H * 1 / 5, (float) v2.getX());
            vertices[3] = new Vector3f((float) v1.getY(), H * 1 / 5, (float) v1.getX());
