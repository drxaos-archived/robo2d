...
function SummerHtmlImageMapCreator() {
    ...
    var app = (function () {
        ...
        function onSvgMousedown(e) {
            if (mode === 'editing') {
                if (e.target.parentNode.tagName === 'g') {
                    ...
                    } else if (e.target.tagName === 'rect' || e.target.tagName === 'circle' || e.target.tagName === 'polygon' || e.target.tagName === 'line') {
                        edit_type = 'move';
            ...
            if (mode === 'drawing' && !is_draw && shape) {
                ...
                switch (shape) {
                    ...
                    case 'polygon':
                        ...
                        break;
                    case 'vector':
                        new_area = new Vector(utils.rightX(e.pageX), utils.rightY(e.pageY));
        ...
        function onAreaDblClick(e) {
            if (mode === 'editing') {
                if (e.target.tagName === 'rect' || e.target.tagName === 'circle' || e.target.tagName === 'polygon' || e.target.tagName === 'line') {
                    selected_area = e.target.parentNode.obj;
    ...
    var from_html_form = (function () {
        var form = utils.id('from_html_wrapper'),
            ...
            close_button = form.querySelector('.close_button'),
            regexp_area = /<area(?=.*? shape="(rect|circle|poly|vector)")(?=.*? coords="([\d ,]+?)")[\s\S]*?>/gmi,
    ...
    var buttons = (function () {
        ...
        vector.addEventListener('click', onShapeButtonClick, false);
    ...
    var Vector = function (x, y) {
        ...
        this.params = {
            x1: x, //distance from the left edge of the image to the start of the vector
            y1: y, //distance from the top edge of the image to the start of the vector
            x2: x, //distance from the left edge of the image to the end of the vector
            y2: y  //distance from the top edge of the image to the end of the vector
        };
        ...
        this.g = document.createElementNS('http://www.w3.org/2000/svg', 'g'); //container
        this.rect = document.createElementNS('http://www.w3.org/2000/svg', 'line'); //rectangle
        this.rect.setAttribute('marker-end', "url(#triangle)");
        this.rect.setAttribute('stroke', "black");
        this.rect.setAttribute('stroke-width', "8");
        app.addNodeToSvg(this.g);
        this.g.appendChild(this.rect);
        ...
        this.helpers = { //object with all helpers-rectangles
            start: new Helper(this.g, x, y),
            end: new Helper(this.g, x, y)
        };

        this.helpers.start.setAction('move').setCursor('move');
        this.helpers.end.setAction('editEnd').setCursor('move');
    ...
    Vector.prototype.setCoords = function (params) {
        this.rect.setAttribute('x1', params.x1);
        this.rect.setAttribute('y1', params.y1);
        this.rect.setAttribute('x2', params.x2);
        this.rect.setAttribute('y2', params.y2);

        this.helpers.start.setCoords(params.x1, params.y1);
        this.helpers.end.setCoords(params.x2, params.y2);

        return this;
    };

    Vector.prototype.setParams = function (params) {
        this.params.x1 = params.x1;
        this.params.y1 = params.y1;
        this.params.x2 = params.x2;
        this.params.y2 = params.y2;

        return this;
    };

    Vector.prototype.redraw = function () {
        this.setCoords(this.params);

        return this;
    };

    Vector.prototype.dynamicDraw = function (x2, y2, square) {
    ...
    Vector.prototype.onDraw = function (e) {
        var _n_f = app.getNewArea(),
            square = e.shiftKey ? true : false;
    ...
    Vector.prototype.onDrawStop = function (e) {
        var _n_f = app.getNewArea(),
            square = e.shiftKey ? true : false;
    ...
    Vector.prototype.move = function (dx, dy) { //offset x and y
        var temp_params = Object.create(this.params);
    ...
    Vector.prototype.editEnd = function (dx, dy) { //offset x and y
        var temp_params = Object.create(this.params);
    ...
    Vector.prototype.dynamicEdit = function (temp_params, save_proportions) {
        this.setCoords(temp_params);

        return temp_params;
    };

    Vector.prototype.onEdit = function (e) {
        var _s_f = app.getSelectedArea(),
            edit_type = app.getEditType(),
            save_proportions = e.shiftKey ? true : false;
    ...
    Vector.prototype.onEditStop = function (e) {
        var _s_f = app.getSelectedArea(),
            edit_type = app.getEditType(),
            save_proportions = e.shiftKey ? true : false;
    ...
    Vector.prototype.select = function () {
        utils.addClass(this.rect, 'selected');

        return this;
    };

    Vector.prototype.deselect = function () {
        utils.removeClass(this.rect, 'selected');

        return this;
    };

    Vector.prototype.with_href = function () {
        utils.addClass(this.rect, 'with_href');

        return this;
    }

    Vector.prototype.without_href = function () {
        utils.removeClass(this.rect, 'with_href');
    ...
    Vector.createFromSaved = function (params) {
        var coords = params.coords,
            href = params.href,
            alt = params.alt,
            title = params.title,
            area = new Vector(coords[0], coords[1]);
