...
public class LevelTest extends RobotTest {
    ...
    public Game createGame() {
        try {
            ...
            MapParser.MapDesc mapDesc = MapParser.parseFodg("locations/levelTest/src/main/java/test1.fodg", 0.003f);

            MapParser.MapObject player = mapDesc.objects.get("player");
            PlayerImpl playerImpl = new PlayerImpl("player1", new KPoint(player.x, player.y), player.r - FastMath.PI / 2);
            game.addPlayer(playerImpl);

            MapParser.MapObject camp = mapDesc.objects.get("camp");
            CampImpl campImpl = new CampImpl(new KPoint(camp.x, camp.y), camp.r - FastMath.PI / 2);
            ...
            MapParser.MapObject controller = mapDesc.objects.get("comp");
            ...
            MapParser.MapPolygon wall1 = mapDesc.polygons.get("wall1");
            game.addWall(new WallImpl(wall1.points, "metal"));

            MapParser.MapObject robot = mapDesc.objects.get("robot");
            RobotImpl robotImpl = new RobotImpl("MR-BS-01", game, playerImpl, new KPoint(robot.x, robot.y), robot.r + FastMath.PI);
